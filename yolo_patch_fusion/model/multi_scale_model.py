...
class MultiPatchInference:
    ...
    def __call__(
        ...
        for i, size in enumerate(patch_sizes):
            results = self.wrapper(image, size, overlap, conf=conf, merging_policy=merging_policy)[0]
            
            if calibrators is not None:
                results = calibrators[i](results)
    ...
    def combine_results(
        ...
        if multiscale_merging_policy == MultiScaleMergingPolicy.score_voting:
            return self._merge_score_voting(scale_results, image)
        
        if multiscale_merging_policy == MultiScaleMergingPolicy.multiplicative_score_voting:
            return self._merge_multiplicative_score_voting(scale_results, image)
        
        if multiscale_merging_policy == MultiScaleMergingPolicy.hybrid_nms:
            return self._merge_hybrid_nms(scale_results, image)
        
        if multiscale_merging_policy == MultiScaleMergingPolicy.hybrid_nms_v2:
            return self._merge_hybrid_nms_v2(scale_results, image)
    ...
    def _merge_score_voting(
        ...
        boxes = torch.tensor(np.concatenate(
            [merged_boxes, merged_scores.reshape(-1, 1), merged_classes.reshape(-1, 1)], axis=1)
        )
        merged_results = Results(orig_img=image, path=None, names=self.wrapper.names)
        merged_results.boxes = Boxes(boxes.reshape(-1, 6), image.shape[:2])
    ...
    def _merge_multiplicative_score_voting(
            self, 
            scale_results: List[Results], 
            image: np.ndarray, 
            iou_threshold: float = 0.5) -> Results:
        ...
        if len(all_boxes) == 0:
            return Results(orig_img=image, path=None, names=self.wrapper.names)   # Если нет детекций, возвращаем пустой объект
        ...
        merged_boxes, merged_scores, merged_classes = multiplicative_score_voting(all_boxes, all_scores, all_classes, iou_threshold)
        
        if len(merged_boxes) == 0:
            return Results(orig_img=image, path=None, names=self.wrapper.names) 
    ...
    def _merge_hybrid_nms(
            self, 
            scale_results: List[Results], 
            image: np.ndarray, 
            iou_threshold: float = 0.5, 
            soft_nms_sigma: float = 0.5) -> Results:
        ...
        merged_boxes, merged_scores, merged_classes = hybrid_nms(all_boxes, all_scores, all_classes, iou_threshold, soft_nms_sigma)
        if len(merged_boxes) == 0:
            return Results(orig_img=image, path=None, names=self.wrapper.names)
    ...
    def _merge_hybrid_nms_v2(
        ...
        merged_boxes, merged_scores, merged_classes = hybrid_nms_v2(all_boxes, all_scores, all_classes, iou_threshold, soft_nms_sigma)
        if len(merged_boxes) == 0:
            return Results(orig_img=image, path=None, names=self.wrapper.names)
...
def multiplicative_score_voting(boxes: np.ndarray, scores: np.ndarray, classes: np.ndarray, iou_threshold: float) -> tuple:
    indices = np.argsort(scores)[::-1]
    merged_boxes = []
    merged_scores = []
    merged_classes = []
    ...
    while len(indices) > 0:
        ...
        merged_boxes.append(weighted_box)
        # merged_scores.append(total_score / len(overlapping_scores))  # Средняя уверенность
        merged_score = 1 - np.prod(1 - np.array(overlapping_scores))
...
def hybrid_nms(boxes: np.ndarray, scores: np.ndarray, classes: np.ndarray, iou_threshold: float, sigma: float) -> tuple:
    indices = np.argsort(scores)[::-1]
    merged_boxes = []
    merged_scores = []
    merged_classes = []
...
def hybrid_nms_v2(boxes: np.ndarray, scores: np.ndarray, classes: np.ndarray, iou_threshold: float, sigma: float) -> tuple:
    indices = np.argsort(scores)[::-1]
    merged_boxes = []
    merged_scores = []
    merged_classes = []
    ...
    while len(indices) > 0:
        ...
        weighted_boxes = [current_box]
        weighted_scores = [current_score]

        for j in range(len(indices)):
            iou = compute_iou(current_box, boxes[indices[j]])
            if iou > iou_threshold and classes[indices[j]] == current_class:
                overlapping_indices.append(indices[j])
                weighted_boxes.append(boxes[indices[j]])
                weighted_scores.append(scores[indices[j]])
            elif 0 < iou <= iou_threshold and classes[indices[j]] == current_class:
                scores[indices[j]] *= np.exp(- (iou ** 2) / sigma)
        ...
        if overlapping_indices:
            ...
            weighted_box = np.sum(weighted_boxes.T * weighted_scores, axis=1) / total_score
            merged_boxes.append(weighted_box)
            # merged_scores.append(total_score / len(weighted_scores))
            merged_score = 1 - np.prod(1 - np.array(weighted_scores))
            merged_scores.append(merged_score)
            merged_classes.append(current_class)

        indices = np.array([idx for idx in indices if idx not in overlapping_indices])
...
class MultiScaleMergingPolicy(str, Enum):
    ...
    hybrid_nms = 'hybrid_nms'
    hybrid_nms_v2 = 'hybrid_nms_v2'
    multiplicative_score_voting = 'multiplicative_score_voting'
